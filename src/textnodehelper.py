...
def split_nodes_delimiter(old_nodes: list[TextNode], delimiter, text_type):
    try:
        ...
        nodes_to_split = ((node, i) for i, node in enumerate(old_nodes) if countOf(node.text, delimiter) % 2 == 1)
        ...
        return split_nodes_delimiter(old_nodes[:first_idx]
                + [TextNode(text, first_type, first_url) for text in first_split[:-1]]
                + [TextNode([TextNode(first_split[-1], first_type, first_url)] + old_nodes[first_idx + 1: second_idx] + [TextNode(second_split[0], second_type, second_url)], text_type)]
                + [TextNode(text, second_type, second_url) for text in second_split[1:]]
                + old_nodes[second_idx + 1:], delimiter, text_type)
