...
def code_block_to_node(block):
    ...
    return ParentNode([
        ParentNode(text_to_html_nodes(text_content), 'code')
    ], 'pre')
...
def unordered_list_block_to_node(block):
    list_items = [ParentNode(text_to_html_nodes(list_item.group(1).strip()), 'li') for list_item in finditer(r'(?m)[-\*](.*)$', block)]
    return ParentNode(list_items, 'ul')

def ordered_list_block_to_node(block):
    list_items = [ParentNode(text_to_html_nodes(list_item.group(1).strip()), 'li') for list_item in finditer(r'(?m)\d+\.(.*)$', block)]
